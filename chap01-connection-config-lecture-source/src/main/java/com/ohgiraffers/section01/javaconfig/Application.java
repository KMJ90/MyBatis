...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        Environment environment
                ...
                , new JdbcTransactionFactory()  // 트렌젝션 매니저 종류 결정
                , new PooledDataSource(DRIVER, URL, USER, PASSWORD));  // ConnectionPool 사용 유무 결정

        Configuration configuration = new Configuration(environment);
        // 설정 객체에 Mapper 등록 (어디에 쿼리를 작성했다를 등록 Mapper 에 대한 메타정보 전달)
        ...
        //           Stream 을 매개변수로 전달하면 SqlSessionFactory 인터페이스 타입의 객체를 반환하는 메소드

        SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(configuration);

        // openSession() : SqlSession 인터페이스 타입의 객체를 반환하는 메소드로 boolean 타입을 인자로 전달
        SqlSession sqlSession = sqlSessionFactory.openSession(false);  // auto commit 에 대한 옵션

        // getMapper() : configuration 에 등록된 Mapper 를 동일 타입에 대해 반환하는 메소드
